---
title: "Exercise: Building a data model"
...
survival analysis, we add two new fields to allow associating a name and
id to each analysis.
...
weight. As before, we add two new fields to allow associating a name and
...
To view how the linking of the tables, we can use a diagram and leverage
the unique identifies to create primary keys in each table and foreign
keys in *analyses* tables by referencing the primary key of the
...
survival_ard <- tidy_estimates |>
  ...
  select(
    name, id, strata, time, n.risk, n.event,
...
save(outputs, file = here("data", "outputs.rda"))
glimpse(outputs)
```
...
First, we extract the tile, labels, and legend information from
*output*. Since this is a simple example, the querying process is
...
Then, we can use {ggplot2} to build the plot.
...
1.  Attempt to create a relation database, using {DBI} and {RSQLite},
