...
```{r}
adtte <- readRDS(here::here("exercises/data/adtte.rds"))
```
...
```{r}
estimates <- survfit(Surv(time = AVAL, event = 1- CNSR) ~ TRTP, 
                     data = adtte)
...
risk_table_plot <- ggsurvtable(risk_table, survtable = "risk.table", ylab = "") +
  scale_y_discrete() 
